...
export default {
  ...
  data() {
    ...
    return {
      dynamicTags: [],
      redirect: 
        this.$route.params.redirect==this.$router.currentRoute.fullPath || typeof(this.$route.params.redirect) == "undefined"?'/':this.$route.params.redirect,
      ruleForm: {
        username: "",
        password: ""
      },
        ...
        if (valid) {
          login(self.$data.ruleForm).then(res => {
            if(res.code && res.code != '200'){
              ...
              console.log(self.$data.redirect)
              console.log(self.$router.currentRoute.fullPath)
